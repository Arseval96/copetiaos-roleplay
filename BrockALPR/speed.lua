...
local radar =
{
	shown = false,
	freeze = false,
	info = "~y~Inicializando ALPR...~w~321...~y~Cargado! ",
	info2 = "~y~Inicializando ALPR...~w~321...~y~Cargado! ",
	minSpeed = 5.0,
	maxSpeed = 75.0,
}
...
Citizen.CreateThread( function()
	...
	while true do
		...
		if IsControlJustPressed(1, 128) and IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) then
			...
			if radar.shown then 
				radar.shown = false 
				radar.info = string.format("~y~Inicializando ALPR...~w~321...~y~Cargado! ")
				radar.info2 = string.format("~y~Inicializando ALPR...~w~321...~y~Cargado! ")
			else 
				radar.shown = true 
			end		
                Wait(75)
		...
		if radar.shown  then
			if radar.freeze == false then
					...
					if IsEntityAVehicle(e) then
						...
						radar.info = string.format("~y~Placa: ~w~%s  ~y~Modelo: ~w~%s  ~y~Velocidad: ~w~%s mph", fplate, fmodel, math.ceil(fvspeed))
					end
					...
					if IsEntityAVehicle(j) then
						...
						radar.info2 = string.format("~y~Placa: ~w~%s  ~y~Modelo: ~w~%s  ~y~Velocidad: ~w~%s mph", bplate, bmodel, math.ceil(bvspeed))
